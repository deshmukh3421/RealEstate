...
const Contact = () => {
    ...
    const onSubmit = async (event) => {
      ...
      formData.append("access_key", "f10a66c3-9b52-484a-aa9b-c5a58408c973");
      ...
      if (data.success) {
        setResult("");
        toast.success("Form Submutted Successfully")
        event.target.reset();
      } else {
      ...
      <form onSubmit={onSubmit} className='max-w-2xl mx-auto text-gray-600 pt-8'>
        <div className='flex flex-wrap'>
            ...
            <div className='w-full md:w-1/2 text-left  md:pl-4'>Your E-mail
                <input className='w-full border border-gray-300 rounded py-3 px-4 mt-2' name='Email' type="email" placeholder='Your E-mail' required/>
            </div>
        </div>
        ...
        <button className='bg-blue-600 text-white py-2 px-12 mb-19 rounded'>{result ? result : "Send Message"}</button>
      </form>
    </motion.div>
  )
}
